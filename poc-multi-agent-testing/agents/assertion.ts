...
export interface AssertionPayload {
  type: 'element_visible' | 'text_contains' | 'element_count' | 'url_matches';
  selector?: string;
...
export class AssertionAgent {
  ...
  async execute(message: AgentMessage<AssertionPayload>): Promise<AgentResult> {
    ...
    try {
      ...
      switch (type) {
        ...
        case 'text_contains':
          if (!expected && !selector) throw new Error("Target or expected required");
          ...
          if (type === 'element_visible') {
            ...
            success = snapText.includes(expected as string);
        ...
        case 'element_count':
            ...
            actualValue = count;
            success = count >= (expected as number); // soft check >= expected
            break;
        ...
        case 'url_matches':
          ...
          success = text.includes(expected as string);
